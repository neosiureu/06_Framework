 ...
 * 
 * 특정사용자가 참여한 채팅방 목록, 각 채팅방의 최근 메시지 , 관련된 정보등등 조회 목적
 * */
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT, -- NULL 이 아닌경우 (로그인한 사람 오픈 멤버일때는 참가자 타겟번호)
		R.OPEN_MEMBER  -- NULL 인경우  (내가오픈맴버가 아닐떄는 OPEN_MEMBER가 타겟번호)
		) TARGET_NO	
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		WHERE R2.CHATTING_ROOM_NO = R.CHATTING_ROOM_NO
		AND R2.OPEN_MEMBER = 1),
