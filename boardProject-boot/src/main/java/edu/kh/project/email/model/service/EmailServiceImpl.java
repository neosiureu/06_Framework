...
public class EmailServiceImpl implements EmailService {
   ...
   public boolean sendTemporaryPasswordEmail(String email, String tempPassword) {
      ...
      try {
         MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true, "UTF-8"); // 헬퍼 생성 (파일 첨부 가능, UTF-8 인코딩)

         helper.setTo(email); // 수신자 설정
         helper.setSubject("[boardProject] 임시 비밀번호 안내입니다."); // 메일 제목 설정

         // 본문 설정 (HTML 템플릿 로드 및 임시 비밀번호 바인딩)
         // loadHtml 호출 시 임시 비밀번호와 함께 호출 ("passwordReset"은 템플릿 파일 이름)
         helper.setText(loadHtml("passwordReset", null, tempPassword), true); // <-- loadHtml 호출: 템플릿 이름, null(authKey), tempPassword 전달

         // 메일 본문에 포함될 이미지 등 추가 (필요시)
         helper.addInline("logo", new ClassPathResource("static/images/logo.jpg")); // 예시: 로고 이미지 첨부

         mailSender.send(mimeMessage); // 실제 메일 발송

         return true; // 메일 발송 성공 시 true 반환

      } catch (MessagingException e) {
         e.printStackTrace(); // 예외 발생 시 로그 출력
         return false; // 메일 발송 실패 시 false 반환
      }
   }


   
   private String loadHtml(String htmlName, String authKey, String tempPassword) {
      Context context = new Context(); // 타임리프 Context 객체 (템플릿에 전달할 변수 담는 상자)

      // Context에 템플릿에서 사용할 변수 설정
      if (authKey != null) { // 인증키가 있다면
         context.setVariable("authKey", authKey);
      }
      if (tempPassword != null) { // 임시 비밀번호가 있다면
         context.setVariable("tempPassword", tempPassword);
      }


      // templates/email/htmlName.html 파일을 읽어와 context에 담긴 변수들을 바인딩하여 HTML 문자열 생성
