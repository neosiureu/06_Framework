...
// - 자원(데이터, 파일) 을 이름(주소)로 하여
// 구분하여 자원Representational의 상태 (State)를 주고 받는것Transfer

// ->    자원의 이름(주소) 를 명시하고
//       HTTP Method(GET, POST , PUT , DELETE)
//      를 이용해 지정된 자원에 대한 CRUD 진행
...
const selectCommentList = () => {
  ...
  //[post,put,DELETE]
  // fetch(주서, {method:"", header:"" , body:""})

  fetch("/comments?boardNo=" + boardNo) //GET방식 요청
  .then(resp => resp.json())
  .then(commentList => {
...
const addComment = document.querySelector("#addComment"); //butoon
...
addComment.addEventListener("click" , () => {
  ...
  .then(result => {
    ...
    if(result > 0){
      ...
      commentContent.value =""; // 작성한 댓글 내용지우기
 ...
 */
 ...
 const insertChildComment = (parentCommentNo,btn) => {
  ...
  if(textarea.value.trim().length == 0){
    alert("내용작성후 등록 버튼을 클릭해주세요!")
    textarea.focus();
    return;
  }

  const data = {
    "commentContent" : textarea.value , // 작성한글
    "memberNo"        : loginMemberNo ,       // 누가 작성했는가?
    "boardNo"       : boardNo ,               // 어느게시글에 달린 댓글인가?
